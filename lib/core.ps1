...
function env { param($name,$value,$targetEnvironment)
    if ( $PSBoundParameters.ContainsKey('targetEnvironment') ) {
        ...
        elseif (($targetEnvironment -eq $null) -or ($targetEnvironment -eq '') -or ($targetEnvironment -eq 'Process') -or ($targetEnvironment -eq 'Session')) { $targetEnvironment = [System.EnvironmentVariableTarget]::Process }
        ...
        elseif (($targetEnvironment -eq 'Global') -or ($targetEnvironment -eq 'Machine')) { $targetEnvironment = [System.EnvironmentVariableTarget]::Machine }
            ...
            warn "WARNING: incorrect targetEnvironment ('$targetEnvironment') parameter to env()"
            $targetEnvironment = [System.EnvironmentVariableTarget]::Process
            }
        }
